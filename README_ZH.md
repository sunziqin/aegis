...
## 📦 模型权重与历史版本矩阵 (Checkpoints & Version Lineage)

| 版本号 | 状态 | 权重 SHA-256 | 独立测试集 Top-1 | 核心说明 |
| :--- | :--- | :--- | :--- | :--- |
| **V4** | ❌ **严重废弃** | `e4879b...` | 35.3% (中文口语) | **已知破损历史旧版**。门控退化（50/51条 risk=0.684 死常数），中文工具概率坍缩。**严禁用于评测。** |
| **V6** | ✅ **正式发布版** | `eaf07edd808cecf43473a13e6a331f57bc8d81696f2a1ed7d82dbc7467e01191` | **84.28%** (N=8,657) | **官方正式 0.5B 旗舰版**。原生 C++ SDPA，校准三门安全闸门，中文工具准确率 99.6%，Sub-50ms 延迟。 |
| **V7** | ⏳ **训练中** | *(Tesla V100 SXM2)* | *(训练中)* | **1.5B 核心基座**。引入对偶置换损失（Permutation Consistency Loss），彻底消除候选位置偏见。 |

> [!WARNING]
> **严禁测试旧版 V4 残留文件**。V4 是早期探索原型，存在门控坍缩与表征未对齐缺陷。任何生产与评测请务必使用经过密码学校验的 **V6** 正式权重。
> 详见 [VERSIONS_ZH.md](VERSIONS_ZH.md) 与 [VERSIONS.md](VERSIONS.md) 获取完整的版本退役与演进报告。

### 📥 官方 V6 权重一键下载与防伪校验

运行官方验证脚本即可自动检测或从 Hub 拉取官方 V6 权重：
